...
from app.weather_service import get_hourly_forecasts, get_weekly_forecasts, DEGREE_SIGN, format_temp, format_hour
...
def test_weekly_forecasts():
    ...
    assert results["city_name"] == "Washington, DC"
    assert len(results["weekly_forecasts"]) == 7
    forecast = results["weekly_forecasts"][0]
